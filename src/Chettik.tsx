import { ArrowLeft, ArrowRight, Check, ChevronLeft, CircleUserRound, Flag, Forward, Lock, Menu, MessageCircle, Moon, Paperclip, Pencil, Pin, Plus, QrCode, Search, Send, Settings, ShieldCheck, ShieldAlert, Smile, Sun, Trash2, Users, X } from 'lucide-react'
...
function Messenger({ account, dark, setDark, language, onLanguage, onLogout }: MessengerProps) {
  ...
  const toggleVoice = () => {
    if (recording) setMessages(old => [...old, { id: crypto.randomUUID(), mine: true, sender: account.name, text: '0:08', time: 'now', reactions: [], kind: 'voice' }])
    ...
    <div className="app-shell">
      ...
      <section className="chat">
        ...
        <form className={`compose ${editing ? 'editing' : ''}`} onSubmit={(e) => { e.preventDefault(); send() }}>
          ...
          <input ref={fileInput} type="file" hidden onChange={e => { attach(e.target.files?.[0]); e.currentTarget.value = '' }} /><button className="icon-btn" type="button" title="Attach file" onClick={() => fileInput.current?.click()}><Paperclip size={19} /></button><button className="icon-btn rich-trigger" type="button" aria-label="Open rich message tools" onClick={() => setRichOpen(true)}><Plus size={19} /></button><input aria-label="Message text" maxLength={4000} value={message} onChange={e => setMessage(e.target.value)} placeholder={language === 'RU' ? 'Написать сообщение…' : 'Write a message…'} /><button className="icon-btn" type="button" aria-label="Open emoji picker" onClick={() => setEmojiOpen(!emojiOpen)}><Smile size={19} /></button>{!message && !editing ? <VoiceButton active={recording} onToggle={toggleVoice} label={recording ? 'Stop voice recording' : 'Record voice message'} /> : <button className="send" aria-label={editing ? 'Save message' : 'Send message'}>{editing ? <Check size={18} /> : <Send size={17} />}</button>}
      ...
      {profileOpen && <ProfilePanel account={profileAccount} onClose={() => setProfileOpen(false)} onBlock={() => { setProfileOpen(false); setConfirm({ action: 'block' }) }} />}
