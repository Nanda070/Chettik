...
const mentionPattern = /(^|[^\w@./])@([a-z0-9_]+)\b/gi

function MessageText({ text, onMention }: { text: string; onMention: (username: string) => void }) {
  const parts: ReactNode[] = []
  let lastIndex = 0
  mentionPattern.lastIndex = 0
  let match: RegExpExecArray | null
  while ((match = mentionPattern.exec(text))) {
    const [token, prefix, username] = match
    const mentionStart = match.index + prefix.length
    if (mentionStart > lastIndex) parts.push(text.slice(lastIndex, mentionStart))
    const mention = `@${username}`
    parts.push(<button className="mention-link" type="button" key={`${mentionStart}-${mention}`} aria-label={`Open ${mention} profile`} onClick={() => onMention(username)}>{mention}</button>)
    lastIndex = mentionStart + mention.length
    if (mentionPattern.lastIndex === match.index + token.length) continue
  }
...
function Messenger({ account, dark, setDark, language, onLanguage, onLogout }: MessengerProps) {
  ...
  const openMention = async (username: string) => {
    if (!token) return
    const mention = `@${username}`
    const response = await fetch(`${API_URL}/users/${encodeURIComponent(username)}/profile`, { headers: { Authorization: `Bearer ${token}` } })
    if (!response.ok) {
      setMentionNotice(`${mention} — user not found`)
      window.setTimeout(() => setMentionNotice(''), 3000)
      return
    }
    setProfileAccount(await response.json() as Account)
    setProfileOpen(true)
  }
  useEffect(() => {
    let disposed = false
  ...
  return <div className={`app ${dark ? 'dark' : ''}`}>
    <div className={`app-shell ${inboxEmpty ? 'inbox-empty-state' : ''}`} style={{ '--sidebar-width': `${chatListWidth}px` } as CSSProperties}>
      ...
      {mentionNotice && <p className="mention-notice" role="status">{mentionNotice}</p>}
