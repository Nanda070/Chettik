...
type Profile = { bio: string; github: string; discord: string; lastSeen: 'Everybody' | 'Contacts' | 'Nobody'; phoneVisibility: 'Everybody' | 'Contacts' | 'Nobody'; blocked: string[] }
...
function load(account: Account) {
  ...
  return { messages: saved.messages?.[account.phone] || initialMessages(account.name), profile: saved.profile?.[account.phone] || { bio: '', github: '', discord: '', lastSeen: 'Contacts', phoneVisibility: 'Contacts', blocked: [] } as Profile, reports: saved.reports || [] as string[] }
}
...
export default function Chettik() {
  ...
  const requestOtp = () => { if (/^\+\d{10,15}$/.test(phone.replace(/\s/g, ''))) setPicked(seedAccounts.find(a => a.phone === phone.replace(/\s/g, '')) ?? { ...seedAccounts[2], phone: phone.replace(/\s/g, ''), name: 'New user', username: '@new' }); else alert(tr.invalid) }
...
function Messenger({ account, dark, setDark, language, onLanguage, onLogout }: MessengerProps) {
  ...
  const send = () => { const text = message.trim(); if (!text || text.length > 4000) return; setMessages(old => [...old, { id: crypto.randomUUID(), mine: true, sender: account.name, text, time: 'now', reactions: [], replyTo: reply?.text }]); setMessage(''); setReply(null) }
  ...
  if (page === 'settings') return <SettingsPage account={account} dark={dark} profile={profile} setProfile={setProfile} onBack={() => setPage('chat')} onLogout={onLogout} />
  ...
  return <div className={`app ${dark ? 'dark' : ''}`}>
    <div className="app-shell">
      ...
      <section className="chat"><header className="chat-head"><button className="icon-btn mobile-menu"><Menu size={20} /></button><div className="avatar" style={{ background: '#6e4c97' }}>M</div><div className="chat-person"><strong>Mark <span className="badge">ADMIN</span></strong><span>online · cloud chat</span></div><div className="head-actions"><button className="icon-btn" title="Switch language" onClick={onLanguage}>{language}</button><button className="icon-btn" title="Settings" onClick={() => setPage('settings')}><Settings size={19} /></button><button className="icon-btn" aria-label="Toggle theme" onClick={() => setDark(!dark)}>{dark ? <Sun size={18} /> : <Moon size={18} />}</button></div></header><div className="messages"><div className="date">Today</div>{matches.map(item => <div className={`message ${item.mine ? 'mine' : ''}`} key={item.id}><div className="avatar" style={{ background: item.mine ? account.color : '#6e4c97' }}>{item.mine ? account.initials : 'M'}</div><div className="bubble">{item.replyTo && <small className="reply-ref">↳ {item.replyTo}</small>}<span className="sender">{item.sender}</span>{item.text}<div className="message-tools"><button onClick={() => setReply(item)} title="Reply"><ArrowLeft size={12} /></button><button onClick={() => setMessages(old => old.map(m => m.id === item.id ? { ...m, reactions: [...m.reactions, '❤️'] } : m))}>❤️ {item.reactions.length || ''}</button><button title="Forward" onClick={() => setMessage(`Fwd: ${item.text}`)}><Forward size={12} /></button><button title="Pin" onClick={() => setMessages(old => old.map(m => m.id === item.id ? { ...m, pinned: !m.pinned } : m))}><Pin size={12} /></button>{item.mine && <button onClick={() => { const next = prompt('Edit message', item.text); if (next?.trim()) setMessages(old => old.map(m => m.id === item.id ? { ...m, text: next.trim(), edited: true } : m)) }}><Pencil size={12} /></button>}<button onClick={() => setReports(old => [...old, `Report: ${item.text}`])}><Flag size={12} /></button>{item.mine && <button onClick={() => deleteMessage(item.id)}><Trash2 size={12} /></button>}</div><span className="meta">{item.pinned ? '📌 ' : ''}{item.time} {item.edited ? 'edited' : ''} {item.mine ? '✓✓' : ''}</span></div></div>)}</div><form className="compose" onSubmit={(e) => { e.preventDefault(); send() }}><input ref={fileInput} type="file" hidden onChange={e => { attach(e.target.files?.[0]); e.currentTarget.value = '' }} /><button className="icon-btn" type="button" title="Attach file" onClick={() => fileInput.current?.click()}><Paperclip size={19} /></button><input maxLength={4000} value={message} onChange={e => setMessage(e.target.value)} placeholder="Write a message…" /><button className="icon-btn" type="button" onClick={() => setMessage(m => `${m}😊`)}><Smile size={19} /></button><button className="send" aria-label="Send message"><Send size={17} /></button>{reply && <span className="compose-reply">Replying to “{reply.text.slice(0, 32)}” <button type="button" onClick={() => setReply(null)}><X size={12} /></button></span>}</form><nav className="mobile-nav"><button className="active"><MessageCircle size={19} />Chats</button><button onClick={() => setPage('settings')}><CircleUserRound size={19} />Profile</button><button onClick={() => setPage('settings')}><Settings size={19} />Settings</button></nav></section>
    </div>
  </div>
}

function SettingsPage({ account, dark, profile, setProfile, onBack, onLogout }: { account: Account; dark: boolean; profile: Profile; setProfile: (p: Profile) => void; onBack: () => void; onLogout: () => void }) {
