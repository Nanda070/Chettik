...
const sessionKey = (account: Account) => `chettik-api-session-${account.phone}`
const credits = {
...
function Messenger({ account, dark, setDark, language, onLanguage, onLogout }: MessengerProps) {
  ...
  const [profile, setProfile] = useState<Profile>(() => ({ bio: '', github: '', discord: '', lastSeen: 'Contacts', phoneVisibility: 'Contacts', blocked: [], privacy: { phone: 'Contacts', lastSeen: 'Contacts', photo: 'Everybody', bio: 'Everybody', birthday: 'Contacts', forwards: 'Everybody', voice: 'Contacts', messages: 'Everybody' }, privacyExceptions: {}, autoDelete: '6 months', pushEnabled: false, telemetryEnabled: false }))
  ...
  const [profilePhoneVisible, setProfilePhoneVisible] = useState(true)
  ...
  const canViewPhone = profileAccount.phone === account.phone
    || (profilePhoneVisible && !phoneExceptions?.never.includes(account.name) && (phoneExceptions?.always.includes(account.name) || phoneAudience === 'Everybody' || (phoneAudience === 'Contacts' && seedAccounts.some(item => item.phone === account.phone))))
  useEffect(() => {
    let disposed = false
    const connect = async () => {
      ...
      setProfile(current => ({ ...current, ...remoteProfile, privacy: { ...current.privacy, ...remoteProfile.privacy }, phoneVisibility: remoteProfile.privacy?.phone || current.phoneVisibility, lastSeen: remoteProfile.privacy?.lastSeen || current.lastSeen }))
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/users/${profileAccount.username.slice(1)}/profile`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(remote => { if (remote) setProfilePhoneVisible(Boolean(remote.phone)) })
      .catch(() => undefined)
